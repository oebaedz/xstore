...
export default function SettingsPage() {
    ...
    const handleHeroUploadSuccess = async (url) => {
        try {
            setLoading(true);
            const { data: prev } = await supabase
                ...
                .single();
            const updatedImages = [...(prev?.value || []), url];
            const { error } = await supabase.from('app_settings')
                .update({ value: updatedImages })
                .eq('key', 'hero_carousel');
            if (error) throw error;
            setCarouselImages(updatedImages);
            showToast('Gambar berhasil ditambahkan ke carousel.', 'success');
        } catch (err) {
            console.error("Error handling hero upload:", err);
            showToast('Gagal menambahkan gambar ke carousel.', 'error');
        } finally {
            setLoading(false);
        }
    }
        ...
        <div className="p-4">
            <div className="flex bg-slate-100 p-1.5 rounded-2xl mb-8">
                <button 
                    ...
                    className={`flex-1 py-3 rounded-xl text-xs md:text-sm font-black uppercase transition-all ${activeTab === 'jadwal' ? 'bg-white shadow-sm text-emerald-500' : 'text-slate-400'}`}
                >
                    <Calendar className="inline mr-1" size={14} /> Jadwal PO
                </button>
                <button 
                    onClick={() => setActiveTab('tampilan')}
                    className={`flex-1 py-3 rounded-xl text-xs md:text-sm font-black uppercase transition-all ${activeTab === 'tampilan' ? 'bg-white shadow-sm text-emerald-500' : 'text-slate-400'}`}
                >
            ...
            {activeTab === 'jadwal' && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <label className="block text-[10px] md:text-sm font-black text-slate-400 uppercase mb-4 tracking-widest">
                    ...
                    <input 
                        type="datetime-local" 
                        className="w-full p-4 text-sm md:text-lg bg-slate-50 border-none rounded-2xl font-bold text-slate-700 focus:ring-2 focus:ring-emerald-500 mb-4"
                    ...
                    <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-2xl">
                        <div>
                        <p className="text-xs md:text-sm font-bold text-emerald-900">Status Toko</p>
                        <p className="text-[10px] md:text-xs text-emerald-500 font-medium">Buka/Tutup PO secara manual</p>
                        </div>
                        <button 
                    ...
                    disabled={loading}
                    className="w-full mt-6 py-4 bg-emerald-900 text-white rounded-2xl font-black uppercase text-xs md:text-sm tracking-widest shadow-xl active:scale-95 transition-all">
            ...
            {activeTab === 'tampilan' && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <label className="text-xs md:text-sm font-black text-slate-400 uppercase tracking-widest">
                        Poster Hero Carousel
                        </label>
                        <span className="text-xs md:text-sm font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-lg">
                    ...
                    <p className="text-xs md:text-sm text-slate-400 italic">
                        *Disarankan menggunakan rasio 16:9.
                    </p>
                    </div>
                </div>
                )}
