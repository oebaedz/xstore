...
const ProductForm = ({ existingProduct, onSaveSuccess }) => {
  ...
  const addVariantField = () => {
    const newVar = { id: Date.now().toString(), name: '', price: 0, image: '' };
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-6 space-y-8 bg-white rounded-3xl border border-slate-100 shadow-xl">
      {/* SECTION 1: INFO UTAMA */}
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <h2 className="flex items-center gap-2 font-bold text-slate-800 text-lg">
            <Package className="w-5 h-5 text-indigo-500" /> Informasi Produk
          </h2>
          <input 
            name="name" value={product.name} onChange={handleMainChange}
            placeholder="Nama Produk (ex: Foto Dua Kiai)" 
            className="w-full p-3 bg-slate-50 text-slate-800 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
            required
          />
          <select 
          ...
          <textarea 
            ...
            className="w-full text-slate-800 p-3 bg-slate-50 border border-slate-200 rounded-2xl h-24 outline-none"
          />
        </div>
      ...
      <section className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <button 
            ...
            className="flex items-center gap-1 text-sm font-bold text-indigo-600 bg-indigo-50 px-4 py-2 rounded-xl hover:bg-indigo-100 transition-all"
          >
        ...
        <div className="space-y-3">
          {product.variants.map((v, index) => (
            <div key={index} className="flex flex-wrap  text-slate-800 md:flex-nowrap gap-3 p-4 bg-slate-50 rounded-2xl items-center border border-slate-100">
              <input 
              ...
              <input 
                ...
                className="flex-1 min-w-[120px] p-2 text-sm bg-white border rounded-lg outline-none"
              />
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-32 pl-8 p-2 text-sm border bg-white rounded-lg outline-none"
                />
              </div>
              {/* <input 
                placeholder="Link Foto Varian" value={v.image}
                onChange={(e) => updateVariantField(index, 'image', e.target.value)}
                className="flex-1 min-w-[150px] p-2 text-sm border rounded-lg outline-none"
              /> */}
              <button 
      ...
      <button 
        ...
        className="w-full py-4 bg-indigo-600 text-white font-bold rounded-2xl flex items-center justify-center gap-2 hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all active:scale-95 disabled:bg-slate-300"
      >
